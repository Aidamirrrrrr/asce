...
import type { SaveRecordNodeData } from "@/lib/flow/flow-schema";
import {
  buildSaveRecordPreview,
  normalizeSaveRecordNodeData,
} from "@/lib/flow/save-record-node-utils";
import { cn } from "@/lib/utils";

export function SaveRecordNode(props: NodeProps) {
  const nodeData = props.data as SaveRecordNodeData;
  const data = normalizeSaveRecordNodeData(nodeData);
  const preview = buildSaveRecordPreview(data);
  const label = data.label || "Запись";
