...
import {
  buildWaitInputPreview,
  normalizeWaitInputNodeData,
} from "@/lib/flow/wait-input-node-utils";
import { cn } from "@/lib/utils";

export function WaitInputNode(props: NodeProps) {
  const nodeData = props.data as WaitInputNodeData;
  const data = normalizeWaitInputNodeData(nodeData);
  const preview = buildWaitInputPreview(data);
  const label = data.label || "Ожидание ввода";
        ...
        >
          <CardContent className={FLOW_NODE_CONTENT_CLASS}>
            <div className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-muted">
              <TextCursorInputIcon className="size-4 text-primary" />
            </div>
